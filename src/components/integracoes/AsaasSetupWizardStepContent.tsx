...
import { AsaasConfigPublic, IntegracaoContaSummary } from "domain/integracoes/types";

type ConnectionState = "idle" | "testing" | "success" | "error";

export type AsaasSetupWizardStepKey = "welcome" | "credentials" | "toggles" | "webhook" | "done";

type AsaasSetupWizardStepContentProps = {
  step: AsaasSetupWizardStepKey;
  theme: AmbienteTheme;
...
export default function AsaasSetupWizardStepContent({
  step,
  theme,
  sandboxTheme,
  producaoTheme,
  ...
  webhookMutation,
  revealMutation,
}: AsaasSetupWizardStepContentProps) {
  function connectionPanelBg() {
    ...
    if (connection === "testing") return `linear-gradient(135deg, ${theme.light} 0%, #f4f1e6 100%)`;
  ...
  function apiKeyInputValue() {
    ...
    if (initial.apiKeyPreenchida) return initial.apiKey;
    return "";
  }
  ...
  function toggleApiKeyVisibility() {
    ...
    if (initial.apiKeyPreenchida && !apiKey) {
    ...
    return (
      <Stack gap={2}>
          ...
          <Stack direction="row" gap={2} alignItems="flex-start">
            ...
            <Box>
              ...
              <Typography variant="body2" color="text.secondary">
                O Asaas recebe cópias dos dados quando os toggles de sincronização estiverem ativos. Você continua
                gerenciando tudo por aqui.
              </Typography>
            </Box>
          </Stack>
        </Paper>
        <Stack direction="row" gap={1} flexWrap="wrap">
          ...
          <Chip icon="mdi:file-sign" color={theme.main} nome="Contratos" />
        </Stack>
        {isLocal && (
          <Alert severity="info" icon={<Icon name="mdi:home-outline" width={22} height={22} />}>
            Ambiente local: use <strong>Sandbox</strong> (laranja) para testar sync de saída. Webhooks só em produção.
          </Alert>
        )}
      </Stack>
    );
    ...
    return (
      <Stack gap={2}>
        ...
        <TextField
          ...
          placeholder={initial.apiKeyPreenchida ? "" : "Cole sua API Key do Asaas"}
          helperText={
            ...
            endAdornment: (
              <InputAdornment position="end">
                <IconButton
                  ...
                  edge="end"
                  disabled={revealMutation.isLoading || (!initial.apiKeyPreenchida && !apiKey)}
                >
        ...
        >
          <Stack direction="row" alignItems="center" gap={2}>
            ...
            <Box sx={{ flex: 1 }}>
              ...
              <Typography variant="body2" color="text.secondary" sx={{ mt: 0.25 }}>
                {connection === "idle" &&
                  `Informe a API Key e teste antes de continuar no ambiente ${theme.label}.`}
                {connection === "testing" && `Validando credenciais em ${initial.runtime.asaasApiBase}…`}
                {connectionDetail}
              </Typography>
            </Box>
          </Stack>
          ...
          {connection === "success" && syncedConta ? (
            <IntegracaoContaSyncPanel providerNome="Asaas" contaFinanceira={syncedConta} />
          ) : null}
          ...
          <Button
            ...
            disabled={testMutation.isLoading || (!apiKey && !initial.apiKeyPreenchida)}
            startIcon={
    ...
    return (
      <Stack gap={1.5}>
        <Typography variant="body2" color="text.secondary" sx={{ mb: 0.5 }}>
          Escolha o que será enviado automaticamente ao Asaas após cada alteração no sistema.
        </Typography>
        {[
          { checked: syncPessoas, set: setSyncPessoas, icon: "groups", label: "Sync Pessoas / Clientes", desc: "CRUD de pessoas → clientes Asaas" },
          { checked: syncFaturas, set: setSyncFaturas, icon: "mdi:file-document-outline", label: "Sync Faturas", desc: "Cada fatura → cobrança no Asaas" },
        ].map((item) => (
          <Paper
            key={item.label}
            elevation={0}
      ...
      <Stack gap={2}>
        <Paper elevation={0} sx={{ p: 2, borderRadius: 2, border: "1px solid #e0e0e0" }}>
          <FormControlLabel
            ...
            label="Receber status de faturas e transferências via webhook"
          />
        ...
        <Button
          ...
          disabled={!webhookHabilitado || webhookMutation.isLoading}
          startIcon={<Icon name="mdi:webhook" width={20} height={20} />}
          sx={{ borderColor: theme.main, color: theme.main }}
        >
          {webhookMutation.isSuccess ? "Webhook configurado" : "Configurar webhook no Asaas"}
        </Button>
        <Alert severity="info">
          Para transferências PIX automáticas, complete depois em Integrações → Asaas o bloco{" "}
          <strong>Validação de saque via Webhook</strong>: copie URL, token e e-mail para{" "}
          <em>Mecanismos de segurança</em>, marque <em>Ativar autorização de saque para estornos Pix</em> e deixe
          desmarcada <em>Validar também saques via interface</em>.
        </Alert>
      </Stack>
    );
  }
