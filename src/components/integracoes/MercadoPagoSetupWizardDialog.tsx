...
import MercadoPagoSetupWizardStepContent from "components/integracoes/MercadoPagoSetupWizardStepContent";
import { MercadoPagoConfigPublic } from "domain/integracoes/types";
import useMercadoPagoSetupWizard from "hooks/useMercadoPagoSetupWizard";
import { useMobileDialog } from "hooks/useMobileDialog";

type MercadoPagoSetupWizardDialogProps = {
  open: boolean;
  onClose: () => void;
  initial: MercadoPagoConfigPublic;
};

export default function MercadoPagoSetupWizardDialog({ open, onClose, initial }: MercadoPagoSetupWizardDialogProps) {
  const mobileDialog = useMobileDialog("sm");
  const wizard = useMercadoPagoSetupWizard({ open, onClose, initial });
  const {
    step,
    setStep,
    theme,
  ...
  return (
    ...
    >
      ...
      >
        <Stack direction="row" alignItems="center" justifyContent="space-between">
          <Stack direction="row" alignItems="center" gap={1.5}>
            ...
            >
              <Icon name="mdi:credit-card-outline" color="#fff" width={26} height={26} />
            </Box>
            <Box>
      ...
      <DialogContent sx={{ bgcolor: "#f4f8fb", minHeight: 300, pt: 2 }}>
        <MercadoPagoSetupWizardStepContent
          step={step}
          theme={wizard.theme}
          ...
          accessToken={wizard.accessToken}
          setAccessToken={wizard.setAccessToken}
          publicKey={wizard.publicKey}
          setPublicKey={wizard.setPublicKey}
          ...
          setWebhookSecret={wizard.setWebhookSecret}
          connection={wizard.connection}
          connectionDetail={wizard.connectionDetail}
          syncedConta={wizard.syncedConta}
          showAccessToken={wizard.showAccessToken}
          setShowAccessToken={wizard.setShowAccessToken}
          showPublicKey={wizard.showPublicKey}
          setShowPublicKey={wizard.setShowPublicKey}
          revealedAccessToken={wizard.revealedAccessToken}
          setRevealedAccessToken={wizard.setRevealedAccessToken}
          revealedPublicKey={wizard.revealedPublicKey}
          setRevealedPublicKey={wizard.setRevealedPublicKey}
          testMutation={wizard.testMutation}
          webhookMutation={wizard.webhookMutation}
          revealAccessTokenMutation={wizard.revealAccessTokenMutation}
          revealPublicKeyMutation={wizard.revealPublicKeyMutation}
        />
      </DialogContent>
